...
internal sealed partial class MainWindow : Window
{
    ...
    private void RenderData(bool forceAutoScale = true, bool preserveZoom = false)
    {
        lock (_plot.Plot.Sync)
        {
            ...
            var cleanPoints = _dataSet.Points.Where(point => point.CleanValid).ToArray();
            ...
            var x = _dataSet.Points.Select(point => (double)point.EspMilliseconds).ToArray();
            var rawSum = _dataSet.Points.Select(point => (double)point.RawSum).ToArray();
            var cleanX = cleanPoints.Select(point => (double)point.EspMilliseconds).ToArray();
            var cleanSum = cleanPoints.Select(point => (double)point.CleanSum).ToArray();
            ...
            {
                var cleanPlot = _plot.Plot.Add.Scatter(cleanX, cleanSum);
                cleanPlot.LegendText = "clean_sum";
            }

            // MA пересчитывается из CSV по clean_sum, чтобы Analyzer не зависел
            // от того, какие вспомогательные поля прислала прошивка.
            if (_showMaCheckBox.IsChecked == true)
            {
                var windowSizes = _dataSet.Points
                    .SelectMany(p => p.EffectiveMovingAverages.Select(ma => ma.WindowSizeSec))
                    .Distinct()
                    .OrderBy(w => w)
                    .ToArray();
                ...
                foreach (var winSize in windowSizes)
                {
                    ...
                    foreach (var point in _dataSet.Points)
                    {
                        var ma = point.EffectiveMovingAverages.FirstOrDefault(m => m.WindowSizeSec == winSize);
                        if (ma != default)
                        {
            ...
            if (preserveZoom)
            {
                // Галочки видимости серий (MA/RAW/Total) — зум не трогаем никогда
        ...
        var cleanPointsForSummary = _dataSet.Points.Where(point => point.CleanValid).ToArray();
        var hardRejectedCount = _dataSet.Points.Count(point => point.RejectReason.Equals("hard_reject", StringComparison.OrdinalIgnoreCase));
        _summaryText.Text =
            $"Строк: {_dataSet.Points.Count} (clean: {cleanPointsForSummary.Length}, hard reject: {hardRejectedCount})\n" +
            ...
            $"clean min/max: {(cleanPointsForSummary.Length > 0 ? $"{cleanPointsForSummary.Min(point => point.CleanSum)} / {cleanPointsForSummary.Max(point => point.CleanSum)}" : "- / -")}";
