...
AnsiConsole.MarkupLine("[bold]ScaleData UDP recorder[/]");
...
AnsiConsole.MarkupLine("Ожидаемый пакет: [grey]scale_id,seq,ms,raw1..rawN,raw_sum,kg_sum,flags[/]");
...
static Table CreatePacketTable(SensorCsvPacket? packet)
{
    ...
    table.AddRow("raw", Markup.Escape(string.Join(", ", packet.RawValues)));
