...
`WeightSample` содержит виртуальные значения `raw_sum`, `clean_sum` и `weight`.

Это сделано потому что:

- `raw_sum` нужен для диагностики цифровых выбросов и калибровки суммы;
...
## Как добавить новый HX711 вход

1. Добавить элемент в `config::kLoadCells`.
2. Задать `name`, `dout_pin`, `sck_pin`, `gain`, `enabled`.
...
Остальная рабочая логика не должна получать массив каналов. Новый вход влияет только на физическое чтение и итоговый `raw_sum`.
...
- `kMovingAverageMaxWindow` задает максимальную емкость окна moving average;
- `kHardRejectEnabled` и `kHardRejectDeltaRawSum` задают жесткое отбрасывание цифровых глюков по `raw_sum`;
...
`raw_sum` не сглаживается и остается диагностическим журналом исходного сэмпла. `clean_sum` получается после hard reject и является входом для MA. Если сэмпл получил `invalid|hard_reject`, его нужно оставить в логе, но не класть в кольцевые буферы MA.
